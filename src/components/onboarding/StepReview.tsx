...
export const StepReview: React.FC<StepReviewProps> = ({ data, onNext, onBack }) => {
  ...
  // Get service details with customization
  const allServices = [
    ...medicalServices,
    ...dentalServices,
    ...cosmeticServices,
  ...
  const services = (data.services || []).map((serviceId: string) => {
    ...
    const customDetails = data.servicesDetails?.[serviceId];
    
    if (customDetails) {
      return {
        ...baseService,
        name: customDetails.customName || baseService?.name || serviceId,
        price: customDetails.customPrice || (baseService ? `$${baseService.price}` : ''),
        description: customDetails.customDescription || ''
      };
    }
    ...
    <div>
            ...
            <div key={service.id} className="bg-gray-50 rounded-lg p-4 border border-gray-200">
              <div className="flex justify-between items-start">
                ...
                <div className="text-right ml-4">
                  ...
                  {service.duration && (
                    <p className="text-xs text-gray-600 mt-1">⏱️ {service.duration} min</p>
                  )}
                </div>
              </div>
            </div>
      ...
      <div className="bg-green-50 border-2 border-green-200 rounded-lg p-6 mb-6">
        <div className="flex items-start">
          ...
          <div>
            <h3 className="font-semibold text-green-800 text-lg mb-2">Profile Complete!</h3>
            <p className="text-sm text-green-700">
              Your profile is ready to submit. After approval, you'll be able to
              receive bookings from patients on the Carrotly platform.
            </p>
          </div>
        </div>
      </div>
